...
namespace HospitalManagementSystem1
{
    public partial class DeleteDoctor : Form
    {
        ...
        {
            SqlCommand cmd = new SqlCommand(@"DELETE FROM DoctorTB WHERE UserID IN (SELECT UserID FROM LogINTB WHERE UserName = @userName);DELETE FROM LogINTB WHERE UserName = @userName", con);
